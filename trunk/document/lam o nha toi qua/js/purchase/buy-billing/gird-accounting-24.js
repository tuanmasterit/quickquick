...
Ext.onReady(function(){
    ...
    var obj_rec24 = Ext.data.Record.create(rec24);

    store24 = new Ext.data.GroupingStore({
    ...
    store24.on('load', function(){
        var totalDebit = 0;
        var totalCredit = 0;
        for (var i = 0; i < store24.getCount(); i++) {
            var record = store24.getAt(i);
        ...
        Ext.getCmp('lblDebit_24').setText(number_format_extra(totalDebit, decimals, decimalSeparator, thousandSeparator));
		Ext.getCmp('lblCredit_24').setText(number_format_extra(totalCredit, decimals, decimalSeparator, thousandSeparator));
		store24.sort('original_amount', 'ASC');
    });

    var tbar24 = new Ext.Toolbar({
        items: [{
            ...
            handler: function(){
                ...
                grid24.getStore().reload();
            }
        }]
    });

    var cm24 = new Ext.grid.ColumnModel({
        defaults: {
            sortable: false
        },
    ...
    grid24.getView().getRowClass = function(record, index){
