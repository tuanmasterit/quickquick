...
Ext.onReady(function(){
    ...
    storeExport = new Ext.data.GroupingStore({
    ...
    storeExport.on('load', function(){
		var totalDebit = 0;
        var totalCredit = 0;
        for (var i = 0; i < storeExport.getCount(); i++) {
            var record = storeExport.getAt(i);
		...
		Ext.getCmp('lblDebit_export').setText(number_format_extra(totalDebit, decimals, decimalSeparator, thousandSeparator));
		Ext.getCmp('lblCredit_export').setText(number_format_extra(totalCredit, decimals, decimalSeparator, thousandSeparator));
		storeExport.sort('original_amount', 'ASC');
    });
    ...
    // create the Grid
    gridExport = new Ext.grid.GridPanel({
        title: '',
        store: storeExport,
        cm: cm1,
        tbar: tbar1,
    ...
    gridExport.getView().getRowClass = function(record, index){
